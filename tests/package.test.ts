...
describe("pi package resources", () => {
  it("declares existing Pi resource paths", async () => {
    const pkg = JSON.parse(await readFile("package.json", "utf8")) as { pi: Record<string, string[]>; files: string[] };
    expect(pkg.pi.extensions).toEqual(["./extensions"]);
    expect(pkg.pi.skills).toEqual(["./skills"]);
    expect(pkg.pi.prompts).toEqual(["./prompts"]);
    for (const resourcePath of [...pkg.pi.extensions, ...pkg.pi.skills, ...pkg.pi.prompts, ...pkg.files]) {
